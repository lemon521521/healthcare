 ...
 * Curl请求类
 *
 * @author    chenhongwei <chenhongwei@lashou.com>
 * @copyright 2013-2020 lashou.com
...
// 						ApiLog::addLog('请求接口：'.$url.',错误信息：'.$curl_error.serialize(curl_getinfo($ch)), '', 'error');
					}
					curl_close($ch);
				}
			}
...
// 				ApiLog::addLog('请求接口：'.$url.',错误信息：'.$curl_error.serialize(curl_getinfo($ch)), '', 'error');
	...
	*/
    ...
    public static function curl_post_xml($url, $xml_data, $timeout = 1) {
        ...
        $header[] = "User-Agent: LASHOU Client 1.1";
        ...
        if (!$response) {
            ...
            if (class_exists('ApiLog', false)) {
                //ApiLog::addLog('请求接口：' . $url . ',错误信息：' . $curl_error . serialize($curl_info), '', 'error');
//                 ApiLog::addLog('请求接口：' . $url . ',错误信息：' . $curl_error . ',请求参数:'.  $xml_data, '', 'error');
            }
        }
        curl_close($ch);                                 //关闭curl链接
//         ApiLog::addLog('请求接口：' . $url . ',返回状态：' . $curl_error . ',返回结果:'.  $response, '', 'debug');
        return $response;
    }
