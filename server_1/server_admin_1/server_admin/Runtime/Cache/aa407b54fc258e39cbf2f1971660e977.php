...
<!--- START 以上首页广告不需更改，保证该TPL页内的标签匹配即可 --->

<div class="btn-toolbar" style="margin-bottom:2px;">
    <a href="__URL__/addads" class="btn btn-primary"><i class="icon-plus"></i> 添加广告</a>
...
<form style="margin-bottom:0px" method="GET" action="__URL__/ads" class="form_search">
...
<!-- search END -->
    <div class="block">
        <a href="#page-stats" class="block-heading" data-toggle="collapse">首页广告列表</a>
        <div id="page-stats" class="block-body collapse in">
              ...
              <?php if(is_array($result_list)): $i = 0; $__LIST__ = $result_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$result_list): $mod = ($i % 2 );++$i;?><tr>
              	<td><a href="<?php echo ($result_list["url"]); ?>" target="_blink"><?php echo ($result_list["title"]); ?></a></td>
              	<td><?php if($result_list[img]): ?><img src="__ROOT__<?php echo ($result_list["img"]); ?>" width="200" high="50">
              		<?php else: ?>
              			无<?php endif; ?>	
              	</td>
              	<td>
              		<?php echo ($result_list["sort"]); ?>
              	</td>
              	<td>
              		<?php echo (getindexads($result_list["types"])); ?>
              	</td>
              	<td>
              		<?php echo (date('Y-m-d',$result_list["createtime"])); ?>
              	</td>
              <td>
              	<a href="<?php echo ($result_list["url"]); ?>" target="_blank" title= "预览详情" ><i class="icon-check"></i></a>
				&nbsp;&nbsp;
				<a href="__URL__/editads/id/<?php echo ($result_list["id"]); ?>" title= "修改" ><i class="icon-pencil"></i></a>
				&nbsp;&nbsp;
				<a data-toggle="modal" href="#myModal" title= "删除" ><i class="icon-remove" href="__URL__/del/id/<?php echo ($result_list["id"]); ?>" ></i></a>
			</td>	
