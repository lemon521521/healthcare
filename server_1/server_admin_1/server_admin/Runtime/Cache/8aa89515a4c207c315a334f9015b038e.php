...
<div class="well">
	...
	<div id="myTabContent" class="tab-content">
		  <div class="tab-pane active in" id="home">
           ...
           <form id="tab" method="post" action="__URL__/add" autocomplete="off">				
				<label>医院名</label>
				<input type="text" name="hospital_name" value="" class="input-xlarge" autofocus="true" required="true" >
				<label>医院短标题</label>
				...
				<label>医院地址</label>
				<input type="text" name="address" value="" class="input-xlarge" autofocus="true" required="true" >		
				<label>医院图片</label>
				<div id="J_imageView"></div>
				<input type="hidden" id="url3" name="thumb_img" value="" /> <input type="button" id="image3" value="选择图片" />				
				<label>医院LOGO</label>			
				<div id="J_logo"></div>
				<input type="hidden" id="url4" name="thumb_logo" value="" /> <input type="button" id="image4" value="选择图片" />	
				<label>医院缩略图</label>			
				<div id="J_thumb"></div>
				<input type="hidden" id="url5" name="thumb" value="" /> <input type="button" id="image5" value="选择图片" />					      		
		         <div class="control-group">
	             <label class="control-label" for="regular">简介</label>
	             <div class="controls">
	                 <textarea id="introduction" name="introduction" style="width:500px;height:300px;"></textarea>
	             </div>
        		 </div>	
				<!-- <label>简介图片</label>
				...
				<input type="hidden" id="bulk_upload" name="bulk_upload" value=""> -->
	             ...
	             <div class="controls">
	                 <textarea id="honor_information" name="honor_information" style="width:500px;height:300px;" ></textarea>
	             </div>
        		 </div>	
	        	 <div class="control-group">
	             <label class="control-label" for="regular">特色</label>
	             <div class="controls">
	                 <textarea id="characteristic" name="characteristic" style="width:500px;height:300px;" ></textarea>
	             </div>
        		 </div>				
	        	 <div class="control-group">
	             <label class="control-label" for="regular">医疗团队</label>
	             <div class="controls">
	                 <textarea id="medical_team" name="medical_team" style="width:500px;height:300px;" ></textarea>
	             </div>
        		 </div>	
        		 	<label>自定义标题</label>
				<input type="text" name="high_customers_title" value="" class="input-xlarge" autofocus="true" required="true" >			
						
	        	 <div class="control-group">
	             <label class="control-label" for="regular">自定义内容</label>
	             <div class="controls">
	                 <textarea id="high_customers" name="high_customers" style="width:500px;height:300px;"></textarea>
	             </div>
        		 </div>				
				<label>医院医师</label>	
				<div>
...
/*$("#physician").change(function(){
	var	 pid_text=$("#pid").text();
	var  phy_str=$("#phy_str").val();
	var	 checkText=$("#physician").find("option:selected").text();
		 $("#pid").text(checkText);
	var  n=$("#n").val();
	if(phy_str.indexOf(checkText)!=-1){
		if(n>0){
			//alert(0);
				pid_d=pid_text+"	"+checkText;
				var phy=phy_str+"	"+checkText;
				$("#pid").text(pid_d);
				$("#phy_str").val(phy);
		}
		var k=n+1;
